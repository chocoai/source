 ...
 * @version 2018-10-22
 */
@Table(name="fz_follow_report", alias="a", columns={
		...
		@Column(name="coin_num", attrName="coinNum", label="跟赞总数", comment="总共赠送出去的跟赞数量（不包括赞赏）"),
	}, orderBy="a.name ASC"
)
public class FzFollowReport extends DataEntity<FzFollowReport> {
	@ExcelFields({
			...
			@ExcelField(title="总共赠送出去的跟赞数量（不包括赞赏）", attrName="coinNum",align = ExcelField.Align.CENTER, sort=45),
	})
	...
	private Long coinNum;		// 跟赞总数（不包括赞赏）
	...
	public FzFollowReport() {
		this(null);
	}

	public FzFollowReport(String id){
		super(id);
	}
	...
	@Length(min=0, max=225, message="department长度不能超过 225 个字符")
